...
fn filter_u8_avx512(
    ...
    let filter_buffer = filter.values().inner();

    let chunks = filter_buffer.bit_chunks(filter.offset(), filter.len());
    let count = chunks
        .iter()
        .chain(iter::once(chunks.remainder_bits()))
    ...
    if chunks.remainder_len() > 0 {
        ...
        unsafe {
            let data_chunk = _mm512_maskz_loadu_epi8(
                filter_chunk,
                data_buffer.as_ptr().cast::<i8>().add(input_offset),
            );
            _mm512_mask_compressstoreu_epi8(
                output_data_buffer
                    .as_mut_ptr()
    ...
    PrimitiveArray::<UInt8Type>::new(output_data_buffer.into(), None)
}

fn filter_i32_avx512(
    ...
    chunks.iter().for_each(|filter_chunk| {
        if filter_chunk != 0 {
            for i in 0..4 {
                ...
                unsafe {
                    let data_chunk = _mm512_loadu_epi32(data_buffer.as_ptr().add(input_offset));
    ...
    if chunks.remainder_len() > 0 {
        ...
        if filter_chunk != 0 {
            for i in 0..4 {
                let filter_chunk = ((filter_chunk >> (i * 16)) & 0xFFFF) as u16;
                unsafe {
                    let data_chunk = _mm512_maskz_loadu_epi32(
                        filter_chunk,
                        data_buffer.as_ptr().add(input_offset) as *const i32,
                    );
                    _mm512_mask_compressstoreu_epi32(
                        output_data_buffer
                            .as_mut_ptr()
